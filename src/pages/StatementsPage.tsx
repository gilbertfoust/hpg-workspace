...
import { StatementGenerator } from "@/components/compliance/StatementGenerator";
import { FinancialStatementViewer } from "@/components/compliance/FinancialStatementViewer";
import { FunctionalExpenseEditor } from "@/components/compliance/FunctionalExpenseEditor";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { ChevronRight } from "lucide-react";

const StatementsPage = () => {
  const { data: ngos } = useQuery({
    queryKey: ["ngos_statements"],
    queryFn: async () => {
  ...
  const yearPeriodIds = (periods || []).filter((p) => new Date(p.start_date).getFullYear() === fiscalYear).map((p) => p.id);
      ...
      <div className="space-y-6 p-6">
        ...
        {selectedNgo ? (
          <div className="space-y-6">
            <StatementGenerator ngoId={selectedNgo} fiscalYear={fiscalYear} fiscalPeriodIds={yearPeriodIds} />

            {(statements || []).map((stmt) => (
              <FinancialStatementViewer key={stmt.id} statementType={stmt.statement_type} dataJson={stmt.data_json} fiscalYear={stmt.fiscal_year} />
            ))}

            <FunctionalExpenseEditor ngoId={selectedNgo} fiscalYear={fiscalYear} allocations={{}} onSave={() => {}} />
          </div>
        ) : (
          <p className="text-muted-foreground">Select an NGO to generate statements.</p>
        )}
      </div>
    </MainLayout>
  );
