          ...
          <TabsContent value="tracking">
            <Card><CardContent className="p-0">
              ...
              {requestsLoading ? <div className="py-10 text-center"><Loader2 className="mx-auto h-6 w-6 animate-spin" /></div> : requestSubmissions && requestSubmissions.length > 0 ? (
                <Table><TableHeader><TableRow><TableHead>Request</TableHead><TableHead>NGO</TableHead><TableHead>Intake</TableHead><TableHead>Routed To</TableHead><TableHead>Submitted</TableHead></TableRow></TableHeader><TableBody>
                  {requestSubmissions.map((s) => <TableRow key={s.id}><TableCell className="font-medium">{s.form_templates?.name || "Request"}</TableCell><TableCell>{s.ngo_id ? ngoNameLookup.get(s.ngo_id) || "—" : "—"}</TableCell><TableCell><StatusBadge status={s.intake_status} /></TableCell><TableCell>{s.routed_to_module || "NGO Coordination"}</TableCell><TableCell>{s.submitted_at ? formatDate(s.submitted_at) : formatDate(s.created_at)}</TableCell></TableRow>)}
                </TableBody></Table>
          ...
          <TabsContent value="compliance">
            <Card><CardContent className="p-0">
              ...
              {complianceLoading ? <div className="py-10 text-center"><Loader2 className="mx-auto h-6 w-6 animate-spin" /></div> : compliancePeriods && compliancePeriods.length > 0 ? (
                <Table><TableHeader><TableRow><TableHead>Period</TableHead><TableHead>NGO</TableHead><TableHead>Type</TableHead><TableHead>Due Date</TableHead><TableHead>Status</TableHead></TableRow></TableHeader><TableBody>
                  {compliancePeriods.map((p) => <TableRow key={p.id}><TableCell className="font-medium">{p.period_label}</TableCell><TableCell>{ngoNameLookup.get(p.ngo_id) || "—"}</TableCell><TableCell>{p.period_type}</TableCell><TableCell>{formatDate(p.due_date)}</TableCell><TableCell><StatusBadge status={p.status} /></TableCell></TableRow>)}
                </TableBody></Table>
            ...
            <Card><CardContent className="p-0">
              <div className="border-b px-5 py-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                  {ngos.length > 1 && <select className="h-9 rounded-md border bg-background px-3 text-sm" value={selectedUploadNgoId} onChange={(e) => setSelectedUploadNgoId(e.target.value)}>{ngos.map(ngo => <option key={ngo.id} value={ngo.id}>{ngo.name}</option>)}</select>}
              ...
              {docsLoading ? <div className="py-10 text-center"><Loader2 className="mx-auto h-6 w-6 animate-spin" /></div> : documents && documents.length > 0 ? (
                <Table><TableHeader><TableRow><TableHead>File Name</TableHead><TableHead>NGO</TableHead><TableHead>Category</TableHead><TableHead>Size</TableHead><TableHead>Status</TableHead><TableHead>Uploaded</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader><TableBody>
                  {documents.map(d => <TableRow key={d.id}><TableCell className="font-medium">{d.file_name}</TableCell><TableCell>{d.ngo_id ? ngoNameLookup.get(d.ngo_id) || "—" : "—"}</TableCell><TableCell><Badge variant="outline">{d.category}</Badge></TableCell><TableCell>{formatFileSize(d.file_size)}</TableCell><TableCell><StatusBadge status={d.review_status} /></TableCell><TableCell>{formatDate(d.uploaded_at || d.created_at)}</TableCell><TableCell className="text-right"><div className="inline-flex gap-2"><Button variant="outline" size="sm" onClick={() => previewDocument(d.file_path)}><Eye className="mr-2 h-4 w-4" />View</Button><Button variant="outline" size="sm" onClick={() => downloadDocument(d.file_path, d.file_name)}><Download className="mr-2 h-4 w-4" />Download</Button></div></TableCell></TableRow>)}
                </TableBody></Table>
