...
interface PartnershipWorkItemDialogProps {
  open: boolean;
...
export function PartnershipWorkItemDialog({
  open,
  onOpenChange,
  defaultTitle,
  workItemType,
  ngoId,
  module,
}: PartnershipWorkItemDialogProps) {
  const createWorkItem = useCreateWorkItem();
  const { data: orgUnits } = useOrgUnits();
  const { data: internalUsers } = useInternalUsers();

  const defaultDepartmentId = useMemo(
    () => orgUnits?.find((unit) => unit.department_name.toLowerCase().includes("partnership"))?.id,
    [orgUnits],
  );
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Create Partnership Work Item</DialogTitle>
          <DialogDescription>
            Create a follow-up task linked to this partnership record.
          </DialogDescription>
        </DialogHeader>

        <div className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="partnership-work-title">Title</Label>
            <Input
              id="partnership-work-title"
              value={title}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              <Label htmlFor="partnership-work-due">Due date</Label>
              <Input
                id="partnership-work-due"
                type="date"
                value={dueDate}
          ...
          <div className="flex items-center justify-between rounded-lg border border-border/60 p-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Capture meeting notes or deliverables.</p>
            </div>
          ...
          <div className="flex items-center justify-between rounded-lg border border-border/60 p-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Route MOU or partnership terms.</p>
            </div>
